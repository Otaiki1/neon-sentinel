# 🎮 Neon Sentinel - Player's Bible

> **The Complete Guide to Surviving The Grid**

Welcome, Sentinel. You are an autonomous security program fighting to contain The Swarm—a spreading corruption that threatens to collapse The Grid. This guide will teach you everything you need to know to survive and push deeper into the infected system layers.

---

## 📖 Table of Contents

1. [The Story](#the-story)
2. [How to Play](#how-to-play)
3. [Enemies & Corruption](#enemies--corruption)
4. [System Layers](#system-layers)
5. [Power-Ups & System Exploits](#power-ups--system-exploits)
6. [Scoring System](#scoring-system)
7. [Progression & Strategy](#progression--strategy)
8. [Leaderboards](#leaderboards)
9. [Prestige Mode](#prestige-mode)
10. [Dynamic Difficulty Evolution](#dynamic-difficulty-evolution)
11. [Corruption Meter](#corruption-meter)
12. [Overclock Mode](#overclock-mode)
13. [Mid-Run Challenges](#mid-run-challenges)
14. [Achievements & Badges](#achievements--badges)

---

## 📖 The Story

### The Grid

Inside a collapsing digital megasystem called **The Grid**, autonomous security programs—**Neon Sentinels**—fight to contain a spreading corruption known as **The Swarm**.

### The Infection

Each enemy color represents a deeper layer of system corruption. You don't "beat levels"—you push deeper into infected layers of the system until you hit a system collapse (death). The deeper you go, the more dangerous the corruption becomes.

### Your Mission

As a Neon Sentinel, your mission is to:
- **Survive** as long as possible
- **Push deeper** into corrupted system layers
- **Defeat graduation bosses** to unlock new layers
- **Climb the leaderboards** and become a Grid Defender

---

## 🎮 How to Play

### Controls

#### Desktop
- **WASD** or **Arrow Keys**: Move your Sentinel
- **Spacebar** or **Mouse Click**: Shoot bullets
- **Q**: Activate Overclock Mode
- **ESC**: Pause/Resume game
- **R**: Restart (when game over)
- **M**: Return to Menu

#### Mobile
- **Virtual Joystick**: Move your Sentinel
- **Fire Button**: Shoot
- **Pause Button**: Top-right
- **Landscape mode recommended** for better gameplay

### Gameplay Basics

1. **Movement**: Your Sentinel starts near the bottom-left of the screen. Move with WASD or arrow keys to dodge enemies and their bullets.

2. **Shooting**: Press Spacebar/click on desktop or hold the Fire button on mobile. Bullets travel horizontally to the right to hit enemies spawning from the right side.

3. **Enemies**: Enemies spawn from the right side and move toward you. They bounce off the top, bottom, and left walls. Destroy them with your bullets to score points.

4. **Lives**: You start with **1 life**. Collect **Life Orbs** (purple orbs) to gain 2 additional lives each. There's no cap on lives—collect as many as you can!

5. **Collisions**: If an enemy touches you, you lose **1 life**. The enemy does NOT die—only you take damage. You have a brief invincibility period after taking damage.

6. **Game Over**: The game ends when your lives reach **0**. Your score is automatically submitted to the weekly leaderboard.

---

## 👾 Enemies & Corruption

Each enemy type represents a different level of system corruption. Understanding them is key to survival.

### 🟢 Green Enemies - Broken Data Fragments
- **Points**: 10
- **Health**: 2 (scales with layer)
- **Speed**: 150
- **Behavior**: Basic enemies that move toward you
- **Lore**: Lowest level of corruption—fragmented data packets that have been corrupted
- **Strategy**: Easy to destroy, but they come in large numbers. Focus on clearing them quickly.

### 🟡 Yellow Enemies - Recompiled Attack Routines
- **Points**: 25
- **Health**: 2 (scales with layer)
- **Speed**: 200
- **Behavior**: Faster than green enemies, move more aggressively
- **Lore**: Corrupted security routines that have been recompiled by The Swarm
- **Strategy**: Slightly tougher and faster. Requires 2 hits to destroy.

### 🟡 Yellow Shield Drones - Protective Subroutines
- **Points**: 15
- **Health**: 3
- **Speed**: 100
- **Behavior**: Generates a shield around nearby enemies (50% damage reduction)
- **Strategy**: Prioritize these or tougher enemies take longer to kill

### 🟡 Yellow Echo Enemies - Signal Decoys
- **Points**: 25
- **Health**: 2
- **Speed**: 180
- **Behavior**: Leaves harmless echo copies that fade after 2 seconds
- **Strategy**: Track the real target; echoes are distractions

### 🔵 Blue Enemies - Hijacked Security Bots
- **Points**: 50
- **Health**: 4 (scales with layer)
- **Speed**: 180
- **Behavior**: **CAN SHOOT BACK!** Fires blue bullets at you every 1.5 seconds
- **Bullet Speed**: 250
- **Lore**: Autonomous security bots that have been hijacked by The Swarm
- **Strategy**: **DANGEROUS!** They shoot back, so prioritize dodging their bullets. Requires 4 hits to destroy. This is where most players start dying.

### 🔵 Blue Buff Enemies - Overclock Nodes
- **Points**: 50
- **Health**: 3
- **Speed**: 150
- **Behavior**: Buffs nearby enemies (faster shooting, higher damage)
- **Strategy**: Remove them first to weaken the group

### 🟣 Purple Enemies - Corrupted AI Cores
- **Points**: 100
- **Health**: 6 (scales with layer)
- **Speed**: 220
- **Behavior**: Elite enemies that move quickly and are hard to kill
- **Lore**: High-level corrupted AI cores—the most dangerous non-boss enemies
- **Strategy**: Very tough and fast. Requires 6 hits to destroy. Can drop power-ups when destroyed.

### 🟣 Purple Fragmenters - Fractured Cores
- **Points**: 100
- **Health**: 4
- **Speed**: 200
- **Behavior**: Splits into 3 green enemies on death
- **Strategy**: Decide when to finish them to avoid swarm pressure

### 🔴 Red Enemies - System Guardians (Bosses)
- **Points**: 500
- **Health**: 20 (scales with layer)
- **Speed**: 120
- **Behavior**: Mini-bosses that are very tough to kill
- **Lore**: System guardians that have been fully taken over by The Swarm
- **Strategy**: Extremely tough. Requires 20 hits to destroy. Only spawn in later layers.

### 🎯 Graduation Bosses

**Graduation Bosses** are special bosses that must be defeated to advance to the next system layer. They are:
- **3x larger** than normal bosses
- **10x tougher** (10x health multiplier)
- **Can shoot**: Fires a 3-bullet spread pattern
- **Unique**: Each layer has a specific graduation boss type
- **Required**: You MUST defeat them to progress to the next layer

When a graduation boss spawns:
- Normal enemy spawning stops
- You'll see a red announcement: **"GRADUATION BOSS INCOMING!"**
- Defeat it to unlock the next layer
- You'll see a green announcement: **"BOSS DEFEATED! Advanced to [Layer Name]"**

---

## 🌐 System Layers

The Grid is divided into **6 system layers**. Each layer represents deeper corruption and increasing danger.

### Layer 1: Boot Sector
- **Score Threshold**: 0 (starting layer)
- **Grid Color**: Green
- **Enemies**: Green only
- **Difficulty**: Easy—build confidence here
- **Description**: The entry point into The Grid. Basic corruption fragments.

### Layer 2: Firewall
- **Score Threshold**: 500 points
- **Grid Color**: Yellow
- **Enemies**: Green, Yellow, Yellow Shield, Yellow Echo
- **Difficulty**: Moderate
- **Enemy Health**: +30% more health
- **Spawn Rate**: +20% more enemies
- **Description**: The first defensive layer. Yellow attack routines appear.

### Layer 3: Security Core
- **Score Threshold**: 1,500 points
- **Grid Color**: Blue
- **Enemies**: Green, Yellow, Yellow Echo, Blue, Blue Buff
- **Difficulty**: Hard—**enemies start shooting back!**
- **Enemy Health**: +60% more health
- **Spawn Rate**: +50% more enemies
- **Description**: The security core where hijacked bots fight back. This is where most players start dying.

### Layer 4: Corrupted AI
- **Score Threshold**: 4,000 points
- **Grid Color**: Purple
- **Enemies**: Green, Yellow, Yellow Echo, Blue, Blue Buff, Purple, Purple Fragmenter
- **Boss Chance**: 1% chance for purple boss
- **Difficulty**: Very Hard
- **Enemy Health**: 2x health
- **Spawn Rate**: 2x enemies
- **Description**: High-level AI cores have been corrupted. Chaos and danger.

### Layer 5: Kernel Breach
- **Score Threshold**: 10,000 points
- **Grid Color**: Red
- **Enemies**: Green, Yellow, Yellow Shield, Yellow Echo, Blue, Blue Buff, Purple, Purple Fragmenter
- **Boss Chance**: 5% chance for mini/medium boss
- **Difficulty**: Extreme
- **Enemy Health**: 2.5x health
- **Spawn Rate**: 2.5x enemies
- **Description**: The kernel has been breached. Mini-bosses appear. Screen shakes intensify.

### Layer 6: System Collapse
- **Score Threshold**: 25,000 points
- **Grid Color**: Bright Red
- **Enemies**: Green, Yellow, Yellow Shield, Yellow Echo, Blue, Blue Buff, Purple, Purple Fragmenter
- **Boss Chance**: 10% chance for final boss
- **Difficulty**: Legendary
- **Enemy Health**: 3x health
- **Spawn Rate**: 3x enemies
- **Description**: The final layer. System collapse is imminent. Only the most skilled Sentinels reach here.

### Layer Progression

- **Progress Bar**: A faint progress bar at the bottom of the screen tracks your progress across all layers
- **Layer Markers**: Vertical lines on the progress bar show each layer threshold
- **Graduation Bosses**: To advance to the next layer, you must defeat the graduation boss that spawns when you reach the score threshold

---

## ⚡ Power-Ups & System Exploits

Power-ups are "system exploits" that grant temporary advantages. Most power-ups can drop from purple/red enemies, while **Life Orbs**, **Firepower**, and **Invisibility** can drop from any enemy.

### 🟢 Speed Boost
- **Sprite**: Green power-up icon
- **Duration**: 10 seconds
- **Effect**: Increases movement speed by **1.5x**
- **Strategy**: Great for dodging enemy bullets and escaping tight situations.

### 🟡 Fire Rate Boost
- **Sprite**: Yellow power-up icon
- **Duration**: 10 seconds
- **Effect**: Reduces fire rate delay by **50%** (shoots twice as fast)
- **Strategy**: Essential for dealing with large enemy waves. Maximize damage output.

### 💜 Score Multiplier
- **Sprite**: Purple orb
- **Duration**: 15 seconds
- **Effect**: **2x score multiplier** for all points earned
- **Strategy**: Collect this when you're about to destroy many enemies for maximum points.

### 🤖 Auto-Shoot
- **Sprite**: Green power-up icon
- **Duration**: 5 seconds
- **Effect**: Automatically shoots continuously without pressing Spacebar
- **Strategy**: Short duration but very useful for clearing waves. Let it do the work while you focus on dodging.

### 💎 Life Orb
- **Sprite**: Purple orb with glow
- **Spawn Chance**: 35% chance from all enemies
- **Effect**: Grants **+2 lives** (no cap—collect as many as you can!)
- **Despawn**: Disappears after 6 seconds if not collected
- **Strategy**: **PRIORITIZE THESE!** Lives are your most valuable resource. Collect every Life Orb you see.

### 🔥 Firepower Upgrade
- **Sprite**: Yellow power-up icon
- **Spawn Chance**: 8% chance from all enemies
- **Duration**: 15 seconds
- **Effect**: 
  - Increases firepower level by **0.5** per power-up
  - Requires **2 power-ups** for 1 full level
  - Each level adds more bullets per shot
- **Strategy**: Stack these for devastating multi-bullet attacks. Very rare but powerful.

### 👻 Invisibility
- **Sprite**: Green power-up icon
- **Spawn Chance**: 15% chance from all enemies
- **Duration**: 10 seconds
- **Effect**: Makes you **invisible** and **invincible** to all damage
- **Visual**: Player becomes semi-transparent with pulsing effect
- **Strategy**: Perfect for escaping overwhelming situations or passing through enemy waves safely.

### Power-Up Mechanics

- **Drop Chances**:
  - **Life Orbs**: 35% from any enemy
  - **Firepower**: 8% from any enemy
  - **Invisibility**: 15% from any enemy
  - **Other Power-Ups**: 25% from purple/red enemies or bosses
- **Despawn Timer**: All power-ups (including Life Orbs) disappear after **6 seconds** if not collected
- **Fade Out**: Power-ups start fading at **5 seconds**, then disappear at **6 seconds**
- **Floating Animation**: Power-ups float up and down to make them more visible
- **Priority**: Life Orbs > Invisibility > Firepower > Score Multiplier > Speed/Fire Rate > Auto-Shoot

---

## 📊 Scoring System

### Base Points

- **Green Enemy**: 10 points
- **Yellow Enemy**: 25 points
- **Blue Enemy**: 50 points
- **Purple Enemy**: 100 points
- **Red Boss**: 500 points
- **Graduation Boss**: Double points (varies by type)

### Combo Multiplier

- **Starting Combo**: 1.0x
- **Combo Increase**: +0.1x for each enemy destroyed
- **Combo Reset**: Resets to 1.0x if you get hit
- **Combo Decay**: Slowly decays if you go too long without scoring
- **Strategy**: Avoid getting hit to maintain high combos for maximum points!

### Score Multiplier Power-Up

- **Effect**: 2x multiplier for 15 seconds
- **Stacking**: Combines with combo multiplier
- **Example**: 2x score power-up + 2.0x combo = **8x total multiplier**

### Final Score Calculation

```
Final Score = Base Points × Combo Multiplier × Score Multiplier × Corruption Multiplier
```

---

## 🎯 Progression & Strategy

### Early Game (Layers 1-2)

**Focus**: Build score and collect Life Orbs
- Destroy green enemies quickly to build combo
- Collect every Life Orb you see
- Learn enemy movement patterns
- Don't rush—survival is more important than speed

### Mid Game (Layers 3-4)

**Focus**: Survive blue enemies and manage resources
- **Blue enemies shoot back!** Prioritize dodging their bullets
- Use Speed Boost to escape dangerous situations
- Fire Rate Boost is essential for dealing with waves
- Save Invisibility for overwhelming moments

### Late Game (Layers 5-6)

**Focus**: Defeat bosses and maximize score
- Bosses are extremely tough—be patient
- Stack Firepower Upgrades for maximum damage
- Use Score Multiplier when you're about to destroy many enemies
- Graduation bosses require strategy—learn their patterns

### Endgame (Time-Based Evolution)

- After ~8 minutes, enemies begin coordinating fire and using formations
- After ~15 minutes, adaptive behavior adjusts spawn lanes based on movement habits
- Movement variety helps reduce predictive accuracy and adaptive pressure

### General Tips

1. **Movement**: Always keep moving. Standing still is death.
2. **Bullet Management**: Your bullets travel right—position yourself to hit multiple enemies
3. **Life Management**: Collect Life Orbs whenever possible. Lives are your most valuable resource.
4. **Power-Up Timing**: Save powerful power-ups (Invisibility, Score Multiplier) for critical moments
5. **Layer Awareness**: Know which layer you're in and what enemies to expect
6. **Boss Strategy**: Graduation bosses are 3x larger and 10x tougher. Focus fire and dodge carefully.

---

## 🏆 Leaderboards

### Weekly Leaderboard

- **Reset**: Leaderboard resets every week (ISO week number)
- **Display**: Top 10 players shown on game over screen (auto-hides after a few seconds)
- **Ranking**: Based on highest score
- **Deepest Layer**: Also tracked—shows how far you progressed
- **Prestige**: Prestige level is tracked and displayed with scores

### Hall of Fame (Dedicated Page)

- **Access**: From the main menu’s "Hall of Fame" link
- **Featured Categories**: 3-5 weekly rotating categories (score, endurance, corruption, etc.)
- **All-Time Records**: Inactive categories still show all-time leaders

### Leaderboard Features

- **X Button**: Click the X in the top-right to close the leaderboard
- **Wallet Integration**: Connect your wallet to attach your address to scores
- **Anonymous Mode**: Play without a wallet (scores display as "Anonymous")

### Titles & Achievements

- **Weekly Winner**: Sector Champion
- **Monthly Winner**: Grid Defender
- **Yearly Winner**: Neon Sentinel Prime

---

## 🎨 Visual Indicators

### UI Elements

- **Score**: Top-left, large green text
- **Combo**: Below score, shows current multiplier
- **Layer**: Shows current system layer name
- **Prestige**: Shows current prestige cycle (P0 = none)
- **Lives**: Orb indicators (each orb shows up to 5 lives, max 4 orbs shown)
- **Progress Bar**: Faint bar at bottom tracking layer progress
- **Overclock Bar**: Cyan bar showing active duration
- **Overclock Status**: Remaining charges and cooldown
- **Challenge Banner**: Mid-run objective text with progress bar
- **Achievement Progress**: Pause menu shows milestone progress

### Announcements

- **Red Card**: "GRADUATION BOSS INCOMING!" - Boss is spawning
- **Orange Card**: "BOSS INCOMING!" - Regular boss spawning
- **Green Card**: "BOSS DEFEATED!" - Boss killed, layer advanced

### Visual Effects

- **Grid Color**: Changes based on current layer
- **Screen Flash**: Red flash when boss spawns, green flash when layer advances
- **Prestige FX**: Purple flash + grid hue shift + glitch jitter on prestige
- **Evolution Pressure**: More formations and bullet patterns appear as time passes
- **Corruption FX**: Grid flicker and red tint intensify at higher corruption
- **Screen Shake**: Intensifies in later layers
- **Health Bars**: All enemies have health bars above them (green >60%, yellow 30-60%, red <30%)

---

## 🚀 Advanced Strategies

### Combo Building

- Destroy enemies quickly without getting hit
- Use Fire Rate Boost to maintain high kill rates
- Avoid risky situations that could break your combo

### Power-Up Synergy

- **Speed + Fire Rate**: Maximum offensive capability
- **Score Multiplier + High Combo**: Massive point gains
- **Invisibility + Fire Rate**: Safe damage dealing
- **Firepower Stack**: Collect multiple for devastating multi-bullet attacks

### Boss Fights

- **Graduation Bosses**: Focus all fire on them
- **Regular Bosses**: Clear smaller enemies first, then focus boss
- **Movement**: Circle around bosses, don't stay in one place
- **Patience**: Bosses are tough—don't rush, survive first

### Layer Transitions

- **Preparation**: Stock up on lives before reaching layer thresholds
- **Boss Fights**: Graduation bosses are mandatory—be ready
- **Environment Changes**: Grid color changes signal new layer
- **Difficulty Spike**: Each layer is significantly harder

---

## 📱 Mobile Tips

- **Landscape Mode**: Highly recommended for better gameplay
- **Touch Controls**: Use the joystick to move and the fire button to shoot
- **Pause Button**: Top-right, includes joystick sensitivity settings
- **UI Scaling**: UI elements are scaled down on mobile to reduce obstruction
- **Sprite Scaling**: All sprites are 50% size on mobile for better performance

---

## ❓ Frequently Asked Questions

**Q: How do I advance to the next layer?**  
A: Reach the score threshold for that layer, then defeat the graduation boss that spawns.

**Q: Can I have unlimited lives?**  
A: Yes! There's no cap on lives. Collect as many Life Orbs as you can.

**Q: What happens when I die?**  
A: Your score is automatically submitted to the weekly leaderboard. You can restart or return to the menu.

**Q: Do power-ups stack?**  
A: Some do (Firepower), but most replace the previous one. Check the power-up descriptions above.

**Q: Why can't I progress past a layer?**  
A: You must defeat the graduation boss that spawns when you reach the score threshold.

**Q: How does the combo multiplier work?**  
A: It increases by 0.1x for each enemy destroyed and resets to 1.0x if you get hit.

**Q: What is Prestige Mode?**  
A: Defeat the Layer 6 graduation boss to loop back to Layer 1 with higher difficulty and score multipliers. Each loop increases the prestige level.

---

## 🧬 Prestige Mode

Prestige Mode makes the game endless by looping after Layer 6:

- **Unlock**: Defeat the Layer 6 graduation boss
- **Cycle**: Return to Layer 1 with higher difficulty and score multipliers
- **Scaling**: Multipliers increase each cycle; there’s no cap
- **Visuals**: Grid hue shifts, glitch jitter, and screen flashes intensify
- **Badge**: "Prestige Champion" appears when you reach Prestige 10

---

## 🧠 Dynamic Difficulty Evolution

Enemy behavior evolves over time, not just by score:

- **Phase 1 (0-3 min)**: Simple pursuit and random spawns
- **Phase 2 (3-8 min)**: Predictive movement begins
- **Phase 3 (8-15 min)**: Coordinated fire and formation waves
- **Phase 4 (15+ min)**: Adaptive learning shifts spawn lanes

Tips:
- Change direction often to reduce predictive accuracy
- Avoid camping edges; it biases spawns to trap you
- Watch for formation waves and coordinated fire windows

---

## 🧪 Corruption Meter

Corruption is a global risk-reward meter (0-100) that rises over time:

- **Low (0-25%)**: 1.0x score, standard enemies
- **Medium (25-50%)**: 1.5x score, tougher enemies
- **High (50-75%)**: 2.0x score, aggressive enemies
- **Critical (75-100%)**: 3.0x score, very dangerous enemies

How it changes:
- **Passive rise**: Increases every second
- **Risk bonuses**: Corrupted zones (dense enemies), boss kills, no-hit streaks, high combos
- **Safe play**: Corruption slowly decays if you only farm low-risk enemies

Tips:
- Push corruption when you’re confident for big score gains
- Mix movement patterns to survive at high corruption
- Reset pressure with safer play after mistakes

---

## ⚡ Overclock Mode

Overclock Mode is a manual power surge you can trigger mid-run:

- **Activation**: Press `Q` (desktop)
- **Duration**: 15 seconds of boosted speed, fire rate, and score
- **Risk**: Enemy spawns increase during Overclock
- **Limits**: Cooldown between uses and a max number of activations per run

Tips:
- Use Overclock during dense waves or boss phases for max points
- Don’t trigger it when you’re low on lives or boxed in

---

## 🎯 Mid-Run Challenges

Mid-run challenges are short objectives that appear during a run:

- **Timing**: First challenge after ~1 minute, then every few minutes
- **Examples**: No-shoot survival, clean kill streaks, high-combo holds, bullet dodging
- **Rewards**: Bonus score, extra lives, or temporary multipliers
- **Failure**: No penalties—challenges simply expire

Tips:
- Commit to the objective once it appears for the best reward
- Skip safely if the arena state is too chaotic

---

## 🏅 Achievements & Badges

Achievements unlock badges and cosmetics that persist between runs:

- **Tiers**: Basic, Intermediate, Advanced, Legendary
- **Badges**: Displayed on your Hall of Fame profile
- **Cosmetics**: Select unlocked cosmetics in the Hall of Fame page
- **Progress Alerts**: “About to unlock” warnings appear near 70% progress

Tips:
- Check the pause menu for your closest milestones
- Focus on one goal per run for faster unlocks

---

## 🎯 Final Words

The Grid is collapsing. The Swarm is spreading. As a Neon Sentinel, you are the last line of defense. Push deeper, survive longer, and climb the leaderboards to become a legend.

**Good luck, Sentinel. The Grid depends on you.**

---

*Last Updated: Game Version 1.8*

